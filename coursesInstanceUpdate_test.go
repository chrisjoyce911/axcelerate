...
func TestCoursesService_UpdateInstanceName(t *testing.T) {
	type fields struct {
		StatusCode int
		Body       string
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{instanceID: 422662, activityType: "w", name: "New Name"},
			want: UpdateInstanceDetail{
				Status:     "success",
				Message:    "Workshop Instance Updated.",
				InstanceID: 1901499,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("CoursesService.UpdateInstanceName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("CoursesService.UpdateInstanceName() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("CoursesService.UpdateInstanceName() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
